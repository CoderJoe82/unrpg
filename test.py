...
class RaceSelectionPhase(CreationPhaseBase):
    ...
    def _prepare_display_data(self):
        ...
        for index, value in enumerate(race_data_information):
            ...
            if race_property_dict['header'] == 'Racial Abilities':
            ...
            elif race_property_dict['header'] == 'Resistance Bonuses':
                racial_resistances_dict = value
                racial_resistances = racial_resistances_dict['content']
                for key, value in racial_resistances.items():
                    resistances[key] = value
                print(f"Racial Resistances : {racial_resistances}\n")
                ...
                for key, value in racial_stat_bonuses.items():
                    stats[key] = value
            elif race_property_dict['header'] == 'Racial Traits':
                racial_traits_tuple = race_property_dict['content']
                for index, value in enumerate(racial_traits_tuple):
                    trait_id = value
                    trait_data = ability_compendium[trait_id]
                    traits[trait_data['name']] = trait_data['description']
            
            if abilities:
                racial_abilities.append(abilities)
            if resistances:
                racial_resistances.append(resistances)
            if stats:
                bonus_stats.append(stats)
            if traits:
                racial_traits.append(traits)
